...
st.markdown("""
    ...
    .box-val { font-family: 'Space Grotesk', sans-serif; font-size: 16px; font-weight: 700; color: #ffffff; }
...
# إدارة السايدبار للترقية والتحقق من التفعيل
with st.sidebar:
    st.markdown("<h3 style='font-family: Cairo; text-align: center;'>💼 بوابة حسابك الرقمي</h3>", unsafe_allow_html=True)
    st.write("---")
    if st.session_state['is_pro']:
        st.markdown("<div style='background: rgba(0, 210, 255, 0.1); border: 1px solid #00d2ff; padding: 15px; border-radius: 12px; text-align: center;'><span style='color: #00d2ff; font-weight: bold; font-family: Cairo;'>👑 الباقة الاحترافية (نشط)</span></div>", unsafe_allow_html=True)
...
<div class="hero-container">
    ...
    <div class="hero-subtitle">الجيل القادم من أنظمة التشفير الهجينة المعتمدة على دالة زيتا ومعيار الحماية العسكري AES-256-GCM.</div>
</div>
""", unsafe_allow_html=True)

st.caption(f"🔒 بصمة موجة ريمان النشطة: `{st.session_state['master_key']}`")
...
with tab1:
    ...
    if mode == "نص سري للغاية":
        user_text = st.text_area("أدخل أو الصق المحتوى النصي هنا:", height=100)
    ...
    else: # حماية قسم الملفات للباقة المدفوعة
        if not st.session_state.get('is_pro', False):
            st.warning("🔒 ميزة تشفير الملفات مخصصة لمشتركي الباقة الاحترافية (Pro) فقط.")
            st.markdown("💡 للترقية وتفعيل الحساب، يرجى إدخال كود التفعيل المعتمد في القائمة الجانبية للموقع.")
        else:
            uploaded_file = st.file_uploader("قم برفع الملف هنا:")
...
with tab2:
    ...
    dec_mode = st.pills("نوع البيانات المستهدفة:", ["نص مشفر (HEX)", "ملف محمي (.zeta)"], selection_mode="single", default="نص مشفر (HEX)")
    
    if dec_mode == "نص مشفر (HEX)":
        hex_input = st.text_area("أدخل رمز الـ HEX المراد كشفه هنا:")
        ...
        uploaded_zeta = st.file_uploader("ارفع الملف المشفر (.zeta):", type=["zeta"])
        if st.button("🔓 استعادة الملف الأصلي", use_container_width=True):
...
with tab3:
    ...
    else: st.caption("🪐 السجل فارغ حالياً. قم بالعمليات لتظهر هنا.")
    st.markdown("</div>", unsafe_allow_html=True)

# --- تبويب محلل الشفرات (محمي للباقة الاحترافية) ---
with tab4:
    if not st.session_state.get('is_pro', False):
        st.warning("🎛️ نظام التحليل الجنائي ومحلل الشفرات الذكي مقفل ومخصص لحسابات (Pro) فقط.")
        st.markdown("💡 يرجى ترقية حسابك عبر السايدبار لفتح هذه الأداة الهندسية العكسية.")
    else:
        st.markdown("<div class='saas-card'>", unsafe_allow_html=True)
        st.write("### 🎛️ نظام التحليل الجنائي للمخرجات المشفرة")
        analysis_input = st.text_area("أدخل النص المشفر المراد فحصه عشوائياً:", key="analysis_main_text")
        
        if st.button("🔍 بدء الفحص والتحليل الجنائي", use_container_width=True, key="analysis_btn"):
            if analysis_input:
                ...
                """, unsafe_allow_html=True)
                st.session_state['logs'].append(f"🔍 {time.strftime('%H:%M:%S')} - تم تشغيل فحص جنائي لنص.")
